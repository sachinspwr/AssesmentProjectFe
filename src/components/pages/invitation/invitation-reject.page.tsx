...
// ✅ Extract and store token from URL (before component mounts)
const initializeLinkTokenFromUrl = () => {
  const urlParams = new URLSearchParams(window.location.search);
  const tokenFromUrl = urlParams.get('linkToken');

  if (tokenFromUrl) {
    localStorageService.setItem(StorageKeys.TOKEN, tokenFromUrl);

    // Optional: Clean up URL by removing token param
...
function AssessmentInvitationRejectPage() {
  ...
  const rejectionFormConfig: VFormFields[] = [
    ...
    },
    {
      ...
      position: '2 1 12',
      customContent: (
        <VButton type="submit" isLoading={isRejecting}>
          Submit Rejection
        </VButton>
      ),
    },
      ...
      <div className="w-full max-w-md rounded-xl shadow-sm p-8 text-center border border-gray-200">
        ...
        {(status == 'success' || status == 'error') && (
          <div className="mt-8 pt-6 border-t border-gray-200">
            <VTypography as="p" color="secondary" className="text-sm">
              {status === 'success' ? 'Thank you for your response.' : 'We apologize for the inconvenience.'}
            </VTypography>
          </div>
        )}
